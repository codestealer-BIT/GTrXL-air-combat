...
class F_16env(gym.Env):
    ...
    def calculate_done(self):
        self.count+=1
        done=False if self.count<self.max_count and self.distance>100 else True
        return done
...
def main():
    ...
    target_update=10
    buffer_size=10000
    minimal_size=500#minimal_size必须大于batch_size
    batch_size=32
        ...
        with tqdm(total=int(num_episodes/total_iterations), desc='Iteration %d' % (i+1)) as pbar:
            for i_episode in range(int(num_episodes/total_iterations)):
                ...
                if (i_episode+1) % (num_episodes/total_iterations) == 0:
                    pbar.set_postfix({'episode': '%d' % (num_episodes/total_iterations * i + i_episode+1), 'return': '%.2f' % np.mean(return_list[int(-num_episodes/total_iterations):]),'win rate':'%.3f'% (float(env.success)/(num_episodes/total_iterations)*100)+'%'})
                pbar.update(1)
