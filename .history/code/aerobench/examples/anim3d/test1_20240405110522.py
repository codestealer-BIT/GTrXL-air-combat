...
class F_16env(gym.Env):
    def __init__(self,episodes):
        ...
        self.list=[[] for i in range(episodes)]
        self.success=0
        self.max_count=50
    ...
    def step(self,action,m_state):
        ...
        if self.count==0:
            ...
            self.list[0].append(dict[straight_simulate.__name__])
        else:
    ...
    for i in range(total_iterations):
        ...
        with tqdm(total=int(num_episodes/total_iterations), desc='Iteration %d' % (i+1)) as pbar:
            for i_episode in range(int(num_episodes/total_iterations)):
                ...
                pbar.update(1)
                print(env.list[env.success_list])#在一个iteration结束后输出动作
    skip_override=10#3
