        ...
        with tqdm(total=int(num_episodes/total_iterations), desc='Iteration %d' % (i+1)) as pbar:
            for i_episode in range(int(num_episodes/total_iterations)):
                ...
                if (i_episode+1) % (num_episodes/total_iterations) == 0:
                    pbar.set_postfix({'episode': '%d' % (num_episodes/total_iterations * i + i_episode+1), 'return': '%.3f' % np.mean(return_list[int(-num_episodes/total_iterations):]),'win rate':'%.3f'% (float(env.success)/(num_episodes/total_iterations)*100)+'%'})
                pbar.update(1)
