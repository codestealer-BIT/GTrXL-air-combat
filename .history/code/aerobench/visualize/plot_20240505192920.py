...
def init_plot():#初始化Matplotlib的绘图样式
...
def plot_overhead(run_sim_result, waypoints=None, llc=None):#这个函数是来绘制飞机二维xy行动轨迹的
    ...
    if llc is not None:
        num_vars = len(get_state_names()) + llc.get_num_integrators()#这里前13个状态是subf16里的提的，后面三个是（见controlled_f16--llc.get_integrator_derivatives)
        """    
...
#循环遍历每个飞行器，绘制在地图上轨迹
    for i in range(num_aircraft):
        ...
        ax.plot(xs, ys, '-')#二维xy坐标，绘制实线

        label = 'Start' if i == 0 else None
        ax.plot([xs[0]], [ys[1]], 'k*', ms=8, label=label)#起点处标记一个星号

    if waypoints is not None:
        xs = [wp[0] for wp in waypoints]
        ys = [wp[1] for wp in waypoints]

        ax.plot(xs, ys, 'ro', label='Waypoints')#用红色圆圈标记航点
    ...
    ax.legend()#添加图例，显示飞行器起点和航点

    plt.tight_layout()#调整图表布局，保证图表不重叠
...
def plot_attitude(run_sim_result, title='Attitude History', skip_yaw=True, figsize=(7, 5)):
    ...
    for index, label, color in zip(indices, labels, colors):#迭代多个可迭代对象
        if skip_yaw and index == StateIndex.PSI:
            continue
        
        ys = states[:, index] # 11: altitude (ft)

        ax.plot(times, ys * rad_to_deg_factor, color, label=label)#打印这么一坨角度
