...
class Buffer():
    ...
    def __init__(self, config:dict, observation_space:spaces.Box, action_space_shape:tuple, max_episode_length:int, device:torch.device) -> None:
        ...
        self.batch_size = self.n_workers * self.worker_steps#最多存一轮的
        self.mini_batch_size = self.batch_size // self.n_mini_batches
        self.max_episode_length = max_episode_length
        self.memory_length = config["transformer"]["memory_length"]#32
        self.num_blocks = config["transformer"]["num_blocks"]#4
        self.embed_dim = config["transformer"]["embed_dim"]#64

        # Initialize the buffer's data storage
        self.rewards = np.zeros((self.n_workers, self.worker_steps), dtype=np.float32)
        self.actions = torch.zeros((self.n_workers, self.worker_steps, len(action_space_shape)), dtype=torch.long)#一维的话，len(shape)就是1
    ...
    def mini_batch_generator(self):
        ...
        indices = torch.randperm(self.batch_size)#随机打乱，使样本保持独立同分布2
