...
#
#         u[0] = throttle command 油门  0.0 < u(1) < 1.0
#         u[1] = elevator 升降舵 command in degrees
#         u[2] = aileron 副翼 command in degrees
...
#         x 是一个长度为13的状态向量
#         u 是一个长度为4的控制输入向量
#         model 是用于选择所使用的飞行动力学模型（"stevens" 或 "morelli"）
#         adjust_cy 用于决定是否调整侧向加速度

#         函数输出：
#
#         xd 是状态向量的导数
#         Nz 和 Ny 分别是垂直和水平方向的载荷系数
#         az 和 ay 是垂直和水平方向的加速度
...
def subf16_model(x, u, model, adjust_cy=True):
    ...
    # component build up
    # 根据所选择的模型（"stevens" 或 "morelli"），气动系数如 cxt, cyt, czt 等都会被计算出来。
    ...
    # 使用上述气动系数、机动参数和飞机的当前状态，计算飞机的动力学行为，如速度、角度变化率等
    ...
    # outputs 函数返回了导数状态 xd 以及其他相关的物理量，如 Nz 和 Ny（飞机的法向和侧向载荷因子）。
