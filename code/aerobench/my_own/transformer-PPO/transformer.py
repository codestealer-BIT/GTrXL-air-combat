...
class MultiHeadAttention(nn.Module):#定义多头注意力机制
    ...
    def __init__(self, embed_dim, num_heads):
        ...
        self.num_heads = num_heads#多头注意力机制中头的数量
        self.head_size = embed_dim // num_heads#一个头对应的维数

        assert (
            self.head_size * num_heads == embed_dim
        ), "Embedding dimension needs to be divisible by the number of heads"
        """线性变换层，将输入的嵌入向量进行线性变换，fc_out将多头注意力输出重新组合成单个输出向量的线性层"""
    ...
    def forward(self, values, keys, query, mask):
        ...
        energy = torch.einsum("nqhd,nkhd->nhqk", [queries, keys])#计算查询和键之间的注意力分数
        # queries shape: (N, query_len, heads, heads_dim),
        # keys shape: (N, key_len, heads, heads_dim)
        # energy: (N, heads, query_len, key_len)
        #将掩码位置的值替换为一个极小的值，以便在未来计算时这些位置权重为0
...
class Transformer(nn.Module):
    ...
    def forward(self, h, memories, mask, memory_indices):#段级递归机制
        """
        Arguments:
        ...
        h = self.activation(self.linear_embedding(h))#h是输入，查询

        # Add positional encoding to every transformer block input
        if self.config["positional_encoding"] == "relative":
            pos_embedding = self.pos_embedding(self.max_episode_steps)[memory_indices]#pos_embedding shape:num_workers,memory_length,embed_dim memory_indice:num_workers,memory_length
            memories = memories + pos_embedding.unsqueeze(2) #给每个Transformerblock相同的位置编码！！！ num_workers,memory_length,num_blocks,embedding_dim
            # memories[:,:,0] = memories[:,:,0] + pos_embedding # add positional encoding only to first layer?
        elif self.config["positional_encoding"] == "learned":#只给对应的记忆加上位置编码
        ...
        for i, block in enumerate(self.transformer_blocks):
            out_memories.append(h.detach())#h是段级递归的隐藏记忆，h在block里是段级传递的
            #这个block就是图中的transformer_block
            h, attention_weights = block(memories[:, :, i], memories[:, :, i], h.unsqueeze(1), mask) # args: value, key, query,mask v,k:num_workers,episode_length,embed_dim  q:num_workers,1(unsqueeze),embed_dim
            #key和value都是memories（block形参value,key,query,mask)
            h = h.squeeze()#降维
            if len(h.shape) == 1:
                h = h.unsqueeze(0)
        return h, torch.stack(out_memories, dim=1)#和RNN的返回值一样，最终输出的隐藏记忆，和四个block堆叠而成输出的记忆
